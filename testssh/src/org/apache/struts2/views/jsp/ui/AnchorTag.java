...
public class AnchorTag extends AbstractClosingTag {
    ...
    @Override
    /*
     * used to contorl the <s;a> tage to have priliedge managerment 
     * @see org.apache.struts2.views.jsp.ComponentTagSupport#doEndTag()
     */
   	public int doEndTag() throws JspException {
   		User user=(User)pageContext.getSession().getAttribute("user");
   		
   		String privilegeUrl=this.action;
   		...
   		if(pos>-1){
			...
			if(user.hasPrivilegeByUrl(privilegeUrl)){
				...
				return super.doEndTag();// 如果有权限，就生成输出的<a>
			}else{
				
				return EVAL_PAGE;//如果没权限，继续执行页面
			}
